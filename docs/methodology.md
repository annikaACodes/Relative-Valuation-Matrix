...
These supplemental sources apply only to specific cells where the primary feed has a genuine gap; they are not the source for most of the matrix. The September 8 gap-fill pass added 30 supplemental fiscal-year rows alongside 511 primary forecast rows, and only the missing fields identified on each supplemental row are merged. MarketScreener remains the primary source for the broad dataset. FactSet-derived pages, 10jqka, broker models, issuer filings, and official share-count disclosures are used selectively to recover otherwise unavailable EPS, cash-flow, EBITDA, net-debt, or share inputs. ECB and other dated reference rates are conversion inputs rather than forecast sources.
